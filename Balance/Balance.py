from Balance.BalanceRequest import BalanceRequest
from Utils.Services import Services

class Balance(Services):
    url_api = None
    def __init__(self, url, url_api, token=None, user=None, password=None):
        super(Balance, self).__init__(url, token, user, password)
        if url_api:
            self.url_api = url_api
        else: 
            print("Debe especificar la url_api")
    
    def get_balance(self):
        return BalanceRequest.account_balance(self.url_api, self.get_token())

    def get_balance_by_id(self, idUser):
        return BalanceRequest.account_balance_by_id(self.url_api, self.get_token(), idUser)

    def add_stamps(self, userId, stamps, comment):
        return BalanceRequest.add_stamps(self.url_api, self.get_token(), userId, stamps, comment)

    def remove_stamps(self, userId, stamps, comment):
        return BalanceRequest.remove_stamps(self.url_api, self.get_token(), userId, stamps, comment)